...
class StockPickingWave(models.Model):
    ...
    def _create_shipment_plans(self):
        ...
        moves = self.pickings_products.filtered(
            lambda r: r.state not in ["done", "cancel"]
            and r.location_dest_id.usage == "transit"
            and not r.departure_shipment_id
        )
        origin_address = set(moves.mapped("ship_from_address_id"))
        destination_address = set(moves.mapped("ship_to_address_id"))
        for plan_combination in itertools.product(origin_address, destination_address):
            combine_moves = moves.filtered(
                lambda r: r.ship_from_address_id == plan_combination[0]
                and r.ship_to_address_id == plan_combination[1]
            )
            context = {
            ...
            if len(similar_moves) == 1:
                wiz.write({"shipment_id": similar_moves[0].departure_shipment_id.id})
